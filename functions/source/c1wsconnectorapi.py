...
class CloudOneConnector:
    ...
    def add_connector(self, aws_cross_account_arn, aws_account_id, aws_account_name):
        ...
        aws_connector = deepsecurity.AWSConnector(
            cross_account_role_arn=aws_cross_account_arn,
            workspaces_enabled="True",
            display_name=display_name,
        )
        logger.info(f"Creating connector for arn: {aws_cross_account_arn}")
        try:
            add_connector_response = self.connectorClient.create_aws_connector(
                aws_connector, self.apiVersion
            )
            logger.info("Connector added")
            logger.info(add_connector_response)
        except ApiException as e:
            logger.error(
                f"Exception when calling AWSConnectorsApi.create_aws_connector: {e}"
            )
        except Exception as e:
            logger.error(e)
    ...
    def delete_connector(self, aws_account_id):
        ...
        if connector_id is None:
            logger.info(
                f"No account found in Workload Security for {aws_account_id}, skipping"
            )
            return None
